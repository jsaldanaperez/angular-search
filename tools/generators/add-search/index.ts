...
export default async function (tree: Tree, schema: any) {
  ...
  const newFileContent = insertContent(fileContent, endOfFile, `\ndeclare module '${schema.projectName}/SearchModule'`);
  ...
  const newAppComponentContent = insertContent(appComponentContent, indexOfEndSearch, newRouterOutlet);
...
function addExposesEntry(tree: Tree, projectRoot: string, projectName: string): void {
  const webpackConfigPath = joinPathFragments(projectRoot, 'webpack.config.js');
  const selector = 'ObjectLiteralExpression > :first-child :has([name=exposes]) > ObjectLiteralExpression';
  const fileContent = tree.read(webpackConfigPath).toString();
  const exposesNode = tsquery(fileContent, selector, { visitAllChildren: true })[0]
  ...
  const newContent = insertContent(fileContent, exposesNode.end - 1, exposedSearchModule);
...
function addRoute(tree: Tree, filePath: string, route: string): void {
  ...
  const selector = 'ArrayLiteralExpression :has([name=RouterModule]) > ArrayLiteralExpression';
  const routerModuleArrayNode = tsquery(fileContent, selector, { visitAllChildren: true })[0];
  const routerModuleArrayContent = routerModuleArrayNode.getFullText();
  const routeContent = endsWith(routerModuleArrayContent, ',]') ? route : `,${route}`;
  const newContent = insertContent(fileContent, routerModuleArrayNode.end - 1, routeContent);
  tree.write(filePath, newContent);
}

function insertContent(fileContent: string, position: number, content: string): string {
  return [fileContent.slice(0, position), content, fileContent.slice(position)].join('');
}
